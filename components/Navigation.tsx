...
export function Navigation({ activeSection }: { activeSection: string }) {
  ...
  return (
    <aside className={`navigation ${activeSection === "home" ? "navigation--dark" : "navigation--light"}`}>
      <div className="nav-brand"><button onClick={() => goTo("home")} aria-label="Go to home">JT</button><span /></div>
      ...
      <nav className={open ? "nav-list is-open" : "nav-list"} aria-label="Portfolio sections">
        {sections.map((section) => <button key={section.id} className={activeSection === section.id ? "active" : ""} onClick={() => goTo(section.id)}><span className="nav-dot"/><span>{section.label.split("\n").map((line, i) => <span key={line}>{line}{i === 0 && section.label.includes("\n") && <br/>}</span>)}</span></button>)}
      </nav>
